...
def enforce_accuracy_requirement(metrics: Mapping[str, Any], min_required_accuracy: float, evaluation_split: str) -> None:
    ...
    if acc < float(min_required_accuracy):
        raise RuntimeError(
            f"Required pairwise_match_accuracy >= {min_required_accuracy:.2f} not met; got {acc:.4f}."
        )
...
def run_pipeline_2d(args: argparse.Namespace, run_paths: ResolvedRunPaths) -> None:
    ...
    from ..healingstone2d.preprocess_2d import load_and_preprocess_fragments_2d as load_2d
    from ..healingstone2d.preprocess_2d import set_deterministic_seed_2d as seed_2d
    ...
    edges = {frag.idx: detect_edges(frag.gray, frag.binary) for frag in fragments}
    descriptors = extract_all_descriptors(fragments)

    # Convert descriptors list to dict for matching if needed, 
    # but match_all_fragments takes a list.
    similarity, candidate_matches, pair_scores = match_all_fragments(
        descriptors=descriptors,
        top_k=args.candidate_top_k,
    )

    # Convert candidate_matches (List[FragmentMatch]) to candidate_pairs (List[Tuple[int, int]])
