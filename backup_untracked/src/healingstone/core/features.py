...
from ..ml_models.surface_model import PointSurfaceClassifier, predict_surface_labels
...
def build_point_surface_features(
    points: np.ndarray,
    normals: np.ndarray,
    curvature: np.ndarray,
    normal_var: np.ndarray,
    ...
    """Build input matrix for PointSurfaceClassifier."""
    return np.hstack(
        [
            points,
            normals,
            curvature.reshape(-1, 1),
            normal_var.reshape(-1, 1),
...
def detect_break_surface(
    ...
    surface_model: Optional[PointSurfaceClassifier] = None,
    device: str = "cpu",
) -> Tuple[np.ndarray, np.ndarray, Dict[str, np.ndarray]]:
    """Detect break surfaces via curvature + normal variance + roughness + optional ML model."""
    ...
    if surface_model is not None:
        ft = build_point_surface_features(points, normals, curvature, normal_var, roughness)
        score = predict_surface_labels(surface_model, ft, device=device)
    else:
        score = (
...
def build_fragment_descriptor(
    ...
    if curvature is not None:
        ...
        geom_stats.extend([float(np.mean(curv_sample)), float(np.std(curv_sample))])
        c_hist, _ = np.histogram(_normalize01(curv_sample), bins=8, range=(0, 1), density=True)
        geom_stats.extend(c_hist.tolist())
    else:
        geom_stats.extend([0.0] * 10)

    if roughness is not None:
        rough_sample = roughness[idx]
        geom_stats.extend([float(np.mean(rough_sample)), float(np.std(rough_sample))])
    else:
        geom_stats.extend([0.0] * 2)
