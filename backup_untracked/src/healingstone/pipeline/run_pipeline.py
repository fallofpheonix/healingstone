...
def enforce_accuracy_requirement(metrics: Mapping[str, Any], min_required_accuracy: float, evaluation_split: str) -> None:
    ...
    if not np.isfinite(acc):
        LOG.warning("Pairwise match accuracy is NaN (no labels provided). Accuracy gate skipped.")
        return
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    from ..ml_models.surface_model import collect_pseudo_labels, train_surface_classifier
    ...
    try:
        ...
        LOG.info("Training surface classifier on pseudo-labels...")
        pseudo_x, pseudo_y = collect_pseudo_labels(
            fragments=fragments,
        ...
        surface_model = train_surface_classifier(
            features=pseudo_x,
            labels=pseudo_y,
